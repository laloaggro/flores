import Cart from '../../../frontend/components/cart/Cart.js';
...
describe('Cart Component', () => {
  ...
  test('should handle checkout button click', () => {
    // Mock de carrito con productos
    localStorageMock.getItem.mockImplementation((key) => {
      if (key === 'cart') {
        return JSON.stringify([
    ...
    global.isAuthenticated = jest.fn().mockReturnValue(true);

    const cart = new Cart();
    const element = cart.render();
    
    const checkoutButton = element.querySelector('.checkout-btn');
    checkoutButton.click();
    
    // Verificar que no haya errores al hacer clic en el botón
