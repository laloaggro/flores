...
import { updateCartCount, showNotification, loadImageWithProxy } from './utils.js';
...
function setupEventListeners() {
    ...
    document.addEventListener('click', function(e) {
        ...
        // Continuar comprando
        if (e.target.classList.contains('continue-shopping') || e.target.closest('.continue-shopping')) {
            closeCart();
        }
        
        // Finalizar compra
        if (e.target.classList.contains('checkout-button') || e.target.closest('.checkout-button')) {
            checkout();
        }
    });
    
    // Event listeners para modificar cantidad en el carrito
    document.addEventListener('click', function(e) {
    ...
    document.addEventListener('click', function(e) {
        const cartModal = document.getElementById('cartModal');
        if (cartModal && cartModal.style.display === 'block' && !e.target.closest('.cart-content') && !e.target.closest('.cart-icon')) {
            closeCart();
        }
    });
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', function(e) {
            ...
            if (searchTerm) {
                productManager.currentSearch = searchTerm;
    ...
    if (categoryFilter) {
        categoryFilter.addEventListener('change', function() {
            productManager.currentCategory = this.value;
            productManager.currentPage = 1;
            loadProducts();
        });
    }
...
function addToCart(product) {
    ...
    if (existingItem) {
        ...
        cart.push({
            ...product,
            quantity: 1,
            image: '/api/image-proxy?url=https%3A%2F%2Fimages.unsplash.com%2Fphoto-1593617133396-03503508724d%3Fixlib%3Drb-4.0.3%26auto%3Dformat%26fit%3Dcrop%26w%3D200%26q%3D80' // Imagen por defecto
        });
    }
    ...
    // Si el carrito está abierto, actualizar su contenido
    const cartModal = document.getElementById('cartModal');
...
function renderCartItems() {
    ...
    cart.forEach(item => {
        cartItemsHTML += `
            <div class="cart-item" data-id="${item.id}">
                <div class="item-image">
                    <img src="${loadImageWithProxy(item.image)}" alt="${item.name}">
                </div>
                <div class="item-info">
                    <h4>${item.name}</h4>
                    <div class="item-price">${formatPrice(item.price)}</div>
...
// Función para cargar productos si estamos en la página de productos
function loadProductsIfOnProductsPage() {
...
async function loadProducts() {
    ...
    try {
        // Mostrar mensaje de carga
        productsContainer.innerHTML = '<div class="loading-message">Cargando productos...</div>';
        
        // Cargar productos
        const data = await productManager.loadProducts(
            productManager.currentPage,
            productManager.currentCategory,
            productManager.currentSearch
        );
        
        // Renderizar productos
        productManager.renderProducts(data.products, productsContainer);
        
        // Actualizar información de resultados
        if (resultsInfo) {
            resultsInfo.textContent = `Mostrando ${data.products.length} de ${data.total} productos`;
        }
        ...
        console.error('Error al cargar productos:', error);
        productsContainer.innerHTML = '<div class="error-message">Error al cargar productos. Por favor intente nuevamente.</div>';
    }
}
...
function renderPagination(currentPage, totalPages) {
    ...
    // Botón anterior
    if (currentPage > 1) {
        paginationHTML += `<a href="#" class="page-link" data-page="${currentPage - 1}">&laquo; Anterior</a>`;
    }
    ...
    if (currentPage < totalPages) {
        paginationHTML += `<a href="#" class="page-link" data-page="${currentPage + 1}">Siguiente &raquo;</a>`;
    }
